...
import se.valjoh.aoc2019.geom2d.Turn;
...
public class TestPuzzle11 extends BasePuzzleTest {
  ...
  public void testPart1Example1() {
    ...
    state.onOutput(turnValue(Turn.LEFT)); // turn left and move left to (-1,0)
    state.onOutput(Puzzle11.Color.BLACK.getValue()); // paint (-1,0) black
    state.onOutput(turnValue(Turn.LEFT)); // turn left and move down to (-1,-1)
    state.onOutput(Puzzle11.Color.WHITE.getValue()); // paint (-1,-1) white
    state.onOutput(turnValue(Turn.LEFT)); // turn left and move right to (0,-1)
    state.onOutput(Puzzle11.Color.WHITE.getValue()); // paint (0,-1) white
    state.onOutput(turnValue(Turn.LEFT)); // turn left and move up to (0,0)
    state.onOutput(Puzzle11.Color.BLACK.getValue()); // paint (0,0) black
    state.onOutput(turnValue(Turn.RIGHT)); // turn right and move right to (1,0)
    state.onOutput(Puzzle11.Color.WHITE.getValue()); // paint (1,0) white
    state.onOutput(turnValue(Turn.LEFT)); // turn left and move up to (1,1)
    state.onOutput(Puzzle11.Color.WHITE.getValue()); // paint (1,1) white
    state.onOutput(turnValue(Turn.LEFT)); // turn left and move left to (0,1)

    Assert.assertEquals(state.getRobotPosition(), new Point(0, 1));
    Assert.assertEquals(state.getRobotDirection(), Direction.WEST);
  ...
  private static long turnValue(Turn turn) {
    if (turn == Turn.LEFT) return 0;
    if (turn == Turn.RIGHT) return 1;
